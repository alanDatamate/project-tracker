...
const FetchPendingTasksForDevelopers = () => {
    ...
    const [selectedAssignee, setSelectedAssignee] = useState(null); // Store selected assignees
    ...
    useEffect(() => {
        const fetchAssignees = async () => {
            ...
            try {
                ...
                setError('Failed to fetch assignees for selected project.');
                setAssignees([]); // Clear assignees on error
            }
        };
        fetchAssignees();
    }, [project]);
    ...
    const handleProjectChange = useCallback((selectedProject) => {
        ...
        setSelectedAssignee(null); // Reset assignees when project changes
    }, []);
    ...
    if (error && isModalOpen) {
        <ErrorModal isModalOpen={isModalOpen} closeModal={closeModal} />
    }
                ...
                {project && (
                    <div className='flex gap-3'>
                        <AssigneeList
                            ...
                            project={project}
                        />
                        <button className='text-sm bg-blue-500 hover:bg-blue-600 p-1 text-white'
